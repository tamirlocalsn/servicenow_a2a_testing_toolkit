...
# ServiceNow discovery URL pattern: {base_url}/{agent_id}/well_known/agent_json
...
async def fetch_agent_card(httpx_client) -> AgentCard:
    # Fetch Agent Card directly from ServiceNow discovery URL
    try:
        print(f"Fetching agent card from ServiceNow: {AGENT_CARD_URL}")
        
        # Use the working httpx configuration from our investigation
        headers = {
...
def create_message_body():
    # Construct the message body for A2A SDK (different from direct ServiceNow format)
    ...
    message_body = {
        'message': {
            ...
            'messageId': uuid4().hex,
            'kind': 'message'  # SDK expects 'message', not 'user'
        },
        'context': {
            'contextId': None,
            'taskId': None
...
async def main() -> None:
    print("=== ServiceNow A2A Client (SDK Version with Working httpx) ===")
    ...
    # Use working httpx configuration - don't set headers at client level
    ...
    async with httpx.AsyncClient(timeout=timeout) as httpx_client:
        try:
            ...
            print("\n✅ A2A Client (SDK version) completed successfully!")
